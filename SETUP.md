...
GitHub controls the surrounding page layout. The README uses Markdown, supported HTML, and self-contained SVG assets; it does not depend on CSS, JavaScript, or iframes. Tech Stack and Currently share two border-lined panels. The hero and overview each have matching `-light.svg` and `-dark.svg` files, plus mobile versions. README `<picture>` sources select only by theme, with a light fallback. Both themes use the same 900px-wide layout and shrink to fit narrower screens. Combined theme/viewport sources are not used, preventing a tall mobile asset from being selected when switching GitHub themes. The separate mobile assets remain available but are not referenced. The overview includes Python and AI / LLMs alongside the original 13 technologies.
